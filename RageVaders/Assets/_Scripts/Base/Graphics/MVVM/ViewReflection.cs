...
{
  /// <summary>
  /// Bind <see cref="ViewModelBase"/> property with associated <see cref="View{T}"/> property.
  /// It requires to implement property of the same type and name.
  /// </summary>
  ...
  /// <summary>
  /// Bind <see cref="ViewModelBase"/> button with associated <see cref="View{T}"/> function (OnClick) and boolean property
  ...
  public abstract partial class View<T> where T : ViewModelBase
  {
    ...
    private void CheckForEventHandlerRegistration()
    {
      if (this.GetType()
        ...
        .Any(m => m.IsDefined(typeof(RVRegisterEventHandlerAttribute), false)))
      {
        throw new Exception($"View: '{this.GetType().FullName}' trying to register events handler.\n" +
